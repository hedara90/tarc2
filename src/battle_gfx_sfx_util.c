...
static u16 ConvertColorToSepia(u16 input)
{
    u16 output = 0;
    s32 rVal = input & 0x1F;
    s32 gVal = (input >> 5) & 0x1f;
    s32 bVal = (input >> 10) & 0x1f;
    s32 rValNew = (rVal * Q_8_8(0.393) + gVal * Q_8_8(0.769) + bVal * Q_8_8(0.189)) >> 8;
    s32 gValNew = (rVal * Q_8_8(0.349) + gVal * Q_8_8(0.686) + bVal * Q_8_8(0.168)) >> 8;
    s32 bValNew = (rVal * Q_8_8(0.272) + gVal * Q_8_8(0.534) + bVal * Q_8_8(0.131)) >> 8;
    if (rValNew > 31)
        rValNew = 31;
    if (gValNew > 31)
        gValNew = 31;
...
static u16 ConvertColorToDesaturatedNaive(u16 input)
{
    u16 output = 0;
    s32 rVal = input & 0x1F;
    s32 gVal = (input >> 5) & 0x1f;
    ...
    if (!gSpeciesInfo[species].isPlayer)
    {
        for (u16 i = 0; i < 16; i++)
        {
            if (gSpeciesInfo[species].excludeBlend[i])
                ...
                //palette[i] = ConvertColorToSepia(paletteData[i]);
