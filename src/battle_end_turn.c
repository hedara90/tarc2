...
static bool32 HandleEndTurnAbilities(u32 battler)
{
    ...
    if (SearchTraits(battlerTraits, ABILITY_THE_SPACE_BETWEEN)
     && gStatuses3[battler] & STATUS3_SEMI_INVULNERABLE)
    {
        ...
        if (numBoosts > 0)
        {
            ...
            if (numBoosts > 0)
            {
                ...
                CreateAbilityPopUp(battler, ABILITY_THE_SPACE_BETWEEN, FALSE);
                BattleScriptExecute(BattleScript_TheSpaceBetween);
                effect = TRUE;
            }
        }
    }
