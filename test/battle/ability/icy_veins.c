...
SINGLE_BATTLE_TEST("Icy Veins increases a Sp. Attack by x1.5 in Sun", s16 damage)
{
    u32 move;
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_CHARIZARD) { Ability(ABILITY_ICY_VEINS); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, move); MOVE(player, MOVE_HYPER_VOICE); }
    } SCENE {
        HP_BAR(opponent, captureDamage: &results[i].damage);
        if (move == MOVE_SNOWSCAPE)
            HP_BAR(player);
    } FINALLY {
...
SINGLE_BATTLE_TEST("Icy Veins doesn't increases a Sp. Attack if Cloud Nine/Air Lock is on the field", s16 damage)
{
    u32 move;
    PARAMETRIZE { move = MOVE_CELEBRATE; }
    PARAMETRIZE { move = MOVE_SNOWSCAPE; }
...
SINGLE_BATTLE_TEST("Icy Veins doesn't cause the Pokémon to lose 1/8 max HP if Cloud Nine/Air Lock is on the field")
{
    GIVEN {
        PLAYER(SPECIES_CHARIZARD) { Ability(ABILITY_ICY_VEINS); MaxHP(80); HP(80); }
        OPPONENT(SPECIES_GOLDUCK) { Ability(ABILITY_CLOUD_NINE); }
