...
{
    u32 ability;
    PARAMETRIZE { ability = ABILITY_DISHEARTEN; }
...
SINGLE_BATTLE_TEST("Dishearten (opponent) lowers player's attack after KO", s16 damage)
{
    u32 ability;
    PARAMETRIZE { ability = ABILITY_DISHEARTEN; }
    PARAMETRIZE { ability = ABILITY_SHED_SKIN; }
...
DOUBLE_BATTLE_TEST("Dishearten doesn't activate on an empty field in a double battle")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_EXPLOSION) == EFFECT_EXPLOSION);
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WOBBUFFET) { HP(1); }
        PLAYER(SPECIES_EKANS) { Ability(ABILITY_DISHEARTEN); }
        PLAYER(SPECIES_ABRA);
...
SINGLE_BATTLE_TEST("Dishearten and Eject Button don't force the opponent to Attack")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_EJECT_BUTTON].holdEffect == HOLD_EFFECT_EJECT_BUTTON);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Item(ITEM_EJECT_BUTTON); }
        OPPONENT(SPECIES_HITMONTOP) { Ability(ABILITY_DISHEARTEN); Moves(MOVE_SCRATCH); }
    } WHEN {
        TURN {
...
DOUBLE_BATTLE_TEST("Dishearten activates on an empty slot")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_CROAGUNK);
        PLAYER(SPECIES_WYNAUT);
        PLAYER(SPECIES_HITMONTOP) { Ability(ABILITY_DISHEARTEN); }
...
DOUBLE_BATTLE_TEST("Dishearten activates immediately after the mon was switched in as long as one opposing mon is alive")
{
    GIVEN {
        PLAYER(SPECIES_TAPU_KOKO) { Ability(ABILITY_ELECTRIC_SURGE); };
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_EKANS) { Ability(ABILITY_DISHEARTEN); Item(ITEM_ELECTRIC_SEED); }
...
SINGLE_BATTLE_TEST("Dishearten can not further lower opponents Atk stat if it is at minimum stages")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("Dishearten is not going to trigger if a mon switches out through u-turn and the opposing field is empty")
{
    GIVEN {
        PLAYER(SPECIES_WYNAUT);
        PLAYER(SPECIES_WOBBUFFET);
        ...
        NOT ABILITY_POPUP(playerLeft, ABILITY_DISHEARTEN);
    }
}

SINGLE_BATTLE_TEST("Dishearten activates when it's no longer effected by Neutralizing Gas - switching out")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Dishearten activates when it's no longer affected by Neutralizing Gas - switching moves")
{
    u32 move;
...
SINGLE_BATTLE_TEST("Dishearten activates when it's no longer affected by Neutralizing Gas - opponent caused switches")
{
    u32 move, item;
...
SINGLE_BATTLE_TEST("Dishearten activates when it's no longer affected by Neutralizing Gas - fainted")
{
    GIVEN {
...
DOUBLE_BATTLE_TEST("Dishearten will correctly decrease the attack of the second mon after Protosynthesis activated")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_EKANS) { Ability(ABILITY_DISHEARTEN); }
    } WHEN {
...
DOUBLE_BATTLE_TEST("Dishearten drop down both opposing atk before eject pack has the chance to activate")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT);
...
DOUBLE_BATTLE_TEST("Dishearten will not miss timing for competitive")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_EJECT_PACK); }
