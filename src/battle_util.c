...
bool32 CanAbilityAbsorbMove(u32 battlerAtk, u32 battlerDef, u32 abilityDef, u32 move, u32 moveType, enum AbilityEffectOptions option)
{
    ...
    if ((gAiLogicData->aiCalcInProgress ? AISearchTraits(AIBattlerTraits, ABILITY_MAELSTROM) : SearchTraits(battlerTraits, ABILITY_MAELSTROM) || abilityDef == ABILITY_MAELSTROM)
     && IsWindMove(move) && !(GetBattlerMoveTargetType(battlerAtk, move) & MOVE_TARGET_USER))
    {
        {
            PushTraitStack(battlerDef, ABILITY_MAELSTROM);
            effect = MOVE_ABSORBED_BY_STAT_INCREASE_ABILITY;
            statId = STAT_SPATK;
        }
    }
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        if ((traitCheck = SearchTraits(battlerTraits, ABILITY_MAELSTROM)) && !gSpecialStatuses[battler].switchInTraitDone[traitCheck - 1]
         ...
         && gSideStatuses[GetBattlerSide(battler)] & SIDE_STATUS_TAILWIND)
        {
            ...
            effect += CommonSwitchInAbilities(battler, 0, ABILITY_MAELSTROM, traitCheck, BattleScript_BattlerAbilityStatRaiseOnSwitchIn);
        }
    ...
    case ABILITYEFFECT_ENDTURN:
        if (IsBattlerAlive(battler))
        {
            ...
            if (SearchTraits(battlerTraits, ABILITY_SAND_REPAIR)
             ...
             && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK))
            {
                PushTraitStack(battler, ABILITY_SAND_REPAIR);
                BattleScriptPushCursorAndCallback(BattleScript_SandRepairActivates);
                gBattleStruct->moveDamage[battler] = GetNonDynamaxMaxHP(battler) / 16;
                if (gBattleStruct->moveDamage[battler] == 0)
                    gBattleStruct->moveDamage[battler] = 1;
                gBattleStruct->moveDamage[battler] *= -1;
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        {
            gBattleScripting.moveEffect = MOVE_EFFECT_PARALYSIS;
            gLastUsedAbility = ABILITY_DREAD;
            PREPARE_ABILITY_BUFFER(gBattleTextBuff1, gLastUsedAbility);
            PushTraitStack(gBattlerAttacker, gLastUsedAbility);
            BattleScriptPushCursor();
            gBattlescriptCurrInstr = BattleScript_AbilityStatusEffect;
         ...
         && TryChangeBattleTerrain(gBattlerTarget, STATUS_FIELD_ELECTRIC_TERRAIN, &gFieldTimers.terrainTimer))
        {
            ...
            BattleScriptPushCursor();
            CreateAbilityPopUp(gBattlerAttacker, ABILITY_OVERCHARGE, FALSE);
        ...
        {
            gBattleScripting.moveEffect = MOVE_EFFECT_BURN;
            gLastUsedAbility = ABILITY_SCORCHING_VOLTAGE;
            PREPARE_ABILITY_BUFFER(gBattleTextBuff1, gLastUsedAbility);
            PushTraitStack(gBattlerAttacker, gLastUsedAbility);
            BattleScriptPushCursor();
            gBattlescriptCurrInstr = BattleScript_AbilityStatusEffect;
        ...
        if (SearchTraits(battlerTraits, ABILITY_ATTENTION)
         && gBattlerAttacker != gBattlerTarget
         && gMovesInfo[gCurrentMove].category == DAMAGE_CATEGORY_STATUS)
        {
            gBattleMons[gBattlerTarget].status2 |= STATUS2_ESCAPE_PREVENTION;
            CreateAbilityPopUp(gBattlerAttacker, ABILITY_ATTENTION, FALSE);
            BattleScriptPushCursor();
            gBattlescriptCurrInstr = BattleScript_Attention;
            effect++;
        }
        break;
...
static bool32 IsBattlerGroundedInverseCheck(u32 battler, enum InverseBattleCheck checkInverse, enum IronBallCheck checkIronBall)
{
    ...
    if (gAiLogicData->aiCalcInProgress ? AI_BATTLER_HAS_TRAIT(battler, ABILITY_LUNAR_COLD) : BattlerHasTrait(battler, ABILITY_LUNAR_COLD))
        return FALSE;
    if (TESTING && IS_BATTLER_OF_TYPE(battler, TYPE_FLYING) && (!(checkInverse == INVERSE_BATTLE) || !FlagGet(B_FLAG_INVERSE_BATTLE)))
        return FALSE;
    return TRUE;
}
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, enum ItemHoldEffect holdEffectAtk, u32 weather)
{
    ...
    if (SearchTraits(battlerTraits, ABILITY_STEELWORKER) && moveType == TYPE_STEEL)
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
    if (SearchTraits(battlerTraits, ABILITY_BRUTE_FORCE) && moveType == TYPE_FIGHTING)
        modifier = uq4_12_multiply(modifier, UQ_4_12(TARC_BRUTE_FORCE_MODIFIER));
